...
testInsertManyRecords(void)
{
	...
	TEST_CHECK(initRecordManager(NULL));
	printf("initRecordManager tested successfully.\n");
	TEST_CHECK(createTable("test_table_t",schema));
	printf("createTable tested successfully.\n");
	TEST_CHECK(openTable(table, "test_table_t"));
	printf("openTable tested successfully.\n");
	...
	TEST_CHECK(closeTable(table));
	printf("closeTable tested successfully.\n");
	TEST_CHECK(openTable(table, "test_table_t"));
	printf("openTable tested successfully.\n");

	// retrieve records from the table and compare to expected final stage
        TEST_CHECK(createRecord(&r, schema));    // Added Summer 2021
        printf("create record tested successfully.\n");
	for(i = 0; i < numInserts; i++)
	{
		RID rid = rids[i];
		TEST_CHECK(getRecord(table, rid, r));
		printf("getRecord tested successfully.\n");
		ASSERT_EQUALS_RECORDS(fromTestRecord(schema, realInserts[i]), r, schema, "compare records");
		printf("from Test Record tested successfully.\n");
		printf("%d",i);
	}
        freeRecord(r);
        printf("s");   // Added Summer 2021
